    ...
    return (
        <nav className={`app-sidebar ${isSidebarOpen ? 'open' : ''}`}>
            <div className="d-flex justify-content-between align-items-center mb-3 sidebar-top">
                ...
                <Button variant="link" className="d-lg-none text-dark p-0 sidebar-close-btn" onClick={toggleSidebar}>&times;</Button>
            </div>

            <div className="sidebar-links">
                <LinkContainer to="/dashboard">
                    <Nav.Link className="sidebar-link" onClick={handleNavClick}>Projects</Nav.Link>
                </LinkContainer>
                ...
                <LinkContainer to="/announcements">
                    <Nav.Link className="sidebar-link" onClick={handleNavClick}>📢 Announcements</Nav.Link>
                </LinkContainer>

                {/* --- ATTENDANCE LINKS (Based on Role) --- */}
                
                {/* 1. Admins see "Attendance Logs" */}
                {user?.role === 'ADMIN' ? (
                     <LinkContainer to="/attendance/admin">
                        <Nav.Link className="sidebar-link" onClick={handleNavClick}>
                            📋 Attendance Logs
                        </Nav.Link>
                    </LinkContainer>
                ) : (
                    /* 2. Regular Users see "Attendance" (Mark) */
                    <LinkContainer to="/attendance/mark">
                        <Nav.Link className="sidebar-link" onClick={handleNavClick}>
                            📅 My Attendance
                        </Nav.Link>
                    </LinkContainer>
                ...
                {user?.role === 'ADMIN' && (
                    <>
                        ...
                        <LinkContainer to="/admin/users">
                            <Nav.Link className="sidebar-link" onClick={handleNavClick}>User Management</Nav.Link>
                        </LinkContainer>
                        <LinkContainer to="/admin/teams">
                            <Nav.Link className="sidebar-link" onClick={handleNavClick}>Team Management</Nav.Link>
                        </LinkContainer>
                        <LinkContainer to="/audit-log">
                            <Nav.Link className="sidebar-link" onClick={handleNavClick}>Audit Log</Nav.Link>
                        </LinkContainer>
                    </>
                )}
                    ...
                    {teams.length > 0 ? (
                        teams.map((team) => (
                            <LinkContainer key={team._id} to={`/teams/${team._id}`}>
                                <Nav.Link className="sidebar-link small py-1" onClick={handleNavClick}># {team.name}</Nav.Link>
                            </LinkContainer>
                        ))
                    ) : (
            ...
            <div className="sidebar-user-section">
                ...
                <Button variant="outline-danger" size="sm" className="w-100 mt-2" onClick={handleLogout}>Logout</Button>
            </div>
        </nav>
    );
};
